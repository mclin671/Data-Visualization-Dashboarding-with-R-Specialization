...
#recode values for ideology
cces$party <- recode(cces$ideo5, '1' = 'Very Liberal', '2' = 'Liberal', '3' = 'Moderate', '4' = 'Conservative', '5' = 'Very Conservative')

#make bar plot
ggplot(data = cces, aes(x = party, fill = party)) + geom_bar() + guides(fill = "none") + labs(x = "Ideology", y = "Count", title = "Respondent Ideology")
```
...
#recode religion
cces$religion <- recode(cces$pew_religimp, '1' = 'Very imp.', '2' = 'Somewhat imp.', '3' = 'Not too imp.', '4' = 'Not at all imp.')

#rename and recode ideo5 
cces <- rename(cces, Ideology = ideo5)
cces$Ideology <- recode(cces$Ideology, '1' = 'Very Liberal', '2' = 'Liberal', '3' = 'Moderate', '4' = 'Conservative', '5' = 'Very Conservative')

#plot
ggplot(data = cces, aes(x = religion, fill = Ideology)) + geom_bar() + labs(x = "Importance of Religion", y = "Count", title = "Importance of Religion by Ideology")
```
...
#create dataframe
names <- c("Amanda", "Betty", "Carol")
Student <- rep(names, each = 6)
Semester <- rep(1:6, 3)
Grade <- runif(18, 80, 100)
student_data <- tibble("Semester" = Semester, "Student" = Student, "Grade" = Grade)

#plot
ggplot(student_data, aes(x = Semester, y = Grade, colour = Student)) + geom_line() + facet_wrap(~Student) + labs(title = "Student Grades by Semester")
```
