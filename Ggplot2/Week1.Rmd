...
cel_115 <- filter(cel, congress == 115)

#rename female to Gender & recode values
cel_115 <- rename(cel_115, Gender = female)
cel_115$Gender <- recode(cel_115$Gender, '0' = 'Male', '1' = 'Female')

#plot
ggplot(cel_115, aes(x = dwnom1, y = all_pass, colour = Gender)) + geom_point() + labs(x = "Ideology", y = "Bills Passed")
```
...
#recode majority values
cel_115$majority <- recode(cel_115$majority, '0' = 'Minority', '1' = 'Majority')

#plot
ggplot(cel_115, aes(x = votepct, y = all_pass, colour = Gender)) + geom_point() + labs(x = "Vote Percentage", y = "Bills Passed") + scale_colour_manual(values = c("Orange", "Green")) + facet_wrap(~majority)
```
...
ggplot(cel_115, aes(x = majority, y = les)) + geom_boxplot() + labs(x = "Majority or Minority", y = "Legislative Effectiveness", title = "LES in the 115th Congress")
```
