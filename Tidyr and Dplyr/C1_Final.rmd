...
c1 <- (1:5)
c2 <- (6:10)
my_dat <- data.frame(alpha = c1, beta = c2)
my_dat
```
...
```{r,problem4}
boxplot(dat$faminc_new, main = "Michelle's Boxplot of Family Income", xlab = "Family Income")
```
...
sub1 <- dat %>% filter(gender ==1, region == 1 | region == 2, marstat == 1, newsint == 1)
str(sub1)
```
...
corptax <- recode(dat$CC18_325a, '1' = 1, '2' = 0)
taxded <- recode(dat$CC18_325d, '1' = 1, '2' = 0)
dat <- mutate(dat, tax_scale = corptax + taxded)
dat[1:20,]
```
...
```{r problem9}
dat %>% group_by(region) %>% summarise(mean_approval = mean(CC18_308a))
```
...
```{r problem10}
dat %>% filter(investor == 2 & faminc_new>=5 & faminc_new<=10) %>% summarise(mean_rel = mean(pew_religimp), median_rel = median(pew_religimp), std_rel = sd(pew_religimp))
```
...
```{r problem11}
familyincome <- summarise(dat, mean_inc = mean(faminc_new), median_inc = median(faminc_new), std_inc = sd(faminc_new))

kable(familyincome, format = "simple", caption = "Family Income Distribution")
```
