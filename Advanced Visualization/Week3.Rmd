...
ggplot(data = my_dat, aes(x = Category, y = Value, fill = City)) + geom_bar(stat = 'identity') + labs(x = "Category", y = "Value") + transition_states(City, transition_length = 2) + enter_fade() + exit_fade()
...
#plot
ggplotly(ggplot(data = my_dat, aes(x = Company, y = Output, fill = Company)) + geom_boxplot())
...
plot2 <- ggplot(data = dat3, aes(x = Category, y = Quantity, frame = Time)) + geom_point() + geom_segment(aes(x = Category, xend = Category, y = 0, yend = Quantity)) 
